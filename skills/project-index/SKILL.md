---
name: project-index
description: Builds or rebuilds `.project_index.yaml`, a repository-root map of modules, key files, tags and summaries that lets agents locate relevant files without a full-tree search. Use when the user asks for a project index, when an existing index is stale after a refactor or rename, or when work in a large unfamiliar repository keeps requiring broad searches.
license: MIT
---

# Build a Project Index

Write `.project_index.yaml` at the repository root, then point agents at it from `AGENTS.md`. It is a map, never a specification.
...
You MUST NOT index generated output, vendored dependencies, lock files, binaries, secrets, or `.project_index.yaml` itself. Enumerate candidates with `git ls-files` so ignored paths stay out.
...
`.project_index.yaml` maps modules, key files and tags so you can locate files without a full-tree search.
